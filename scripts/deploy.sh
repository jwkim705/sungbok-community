...
cd /var/jenkins_home/custom/sungbok

# 환경변수 DOCKER_APP_NAME : 컨테이너 메인 이름
DOCKER_APP_NAME=spring-sungbok-back
LOG_FILE=./deploy.log

# 실행중인 blue가 있는지 확인# 프로젝트의 실행 중인 컨테이너를 확인하고, 해당 컨테이너가 실행 중인지 여부를 EXIST_BLUE 변수에 저장
...
# green이 실행중이면 blue up# EXIST_BLUE 변수가 비어있는지 확인
